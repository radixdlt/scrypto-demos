...
blueprint! {
    ...
    impl Escrow {
        ...
        pub fn instantiate_escrow(
            ...
            // Performing the checks to ensure that the two `ResourceSpecifier`s are valid
            assert!(
                to_be_paid_by_party_1.validate().is_ok(),
                "[Instantiation]: The resource specifier of the first party is invalid"
            );
            assert!(
                to_be_paid_by_party_2.validate().is_ok(),
                "[Instantiation]: The resource specifier of the second party is invalid"
            );
            assert_ne!(
                to_be_paid_by_party_1, to_be_paid_by_party_2,
                "[Instantiation]: The two resource specifiers can not be equal"
            );

            // Creating the two `EscrowObligation` structs for the two parties. We will minting
            // the escrow obligation NFTs from these two objects.
            ...
            // Creating the `EscrowObligation` resource and minting two NFTs of the above
            // `EscrowObligation` objects.
            let obligation_nfts: Bucket = ResourceBuilder::new_non_fungible()
                .metadata("name", "Escrow Obligation")
                .metadata("symbol", "ESCROW")
                .metadata(
                    "description",
                    "An NFT that defines your obligation to the escrow account",
                )
                .metadata("team-member-1-github-username", "0xOmarA")
                .initial_supply([
                    (NonFungibleId::from_u32(1), party_1_obligation),
                    (NonFungibleId::from_u32(2), party_2_obligation),
                ]);

            // Instantiating the Escrow component.
            ...
            let component_address: ComponentAddress = Self {
                obligation_non_fungible_resource: obligation_nfts.resource_address(),
                vaults,
            }
            .instantiate()
            .globalize();

            (component_address, obligation_nfts)
        }
        ...
        pub fn deposit(&mut self, obligation_badge: Proof, mut funds: Bucket) -> Bucket {
            // Validating the obligation proof to ensure that it is a of a valid resource address
            let obligation_badge: ValidatedProof = obligation_badge
                .validate_proof(self.obligation_non_fungible_resource)
                .expect("[Deposit]: Invalid obligation proof passed to the method");

            // Getting the data inside the obligation NFT to then make use of it and perform
            // validation using it.
            let obligation: EscrowObligation = obligation_badge.non_fungible().data();
            assert_eq!(
                obligation.amount_to_pay.resource_address(),
                funds.resource_address(),
                "[Deposit]: Bucket passed is of an invalid resource address"
            );

            // Get the vault corresponding to the `amount_to_pay` resource specifier and ensuring
            // that the vault is empty (to make sure that this obligation was not already fulfilled)
            let vault: &mut Vault = self.vaults.get_mut(&obligation.amount_to_pay).unwrap();
            assert!(
                vault.is_empty(),
                "[Deposit]: This obligation has already been fulfilled, deposit not accepted."
            );

            // Take the specified amount of tokens from the funds bucket and deposit them into the
            // vault.
            ...
            // Returning the remaining funds in the bucket
            funds
        }
        ...
        pub fn withdraw(&mut self, obligation_badge: Proof) -> Bucket {
            // Ensure that the escrow has been fulfilled by both parties (i.e. that both parties
            // have paid the amount required by them). If not, then none of the parties can withdraw
            // their funds.
            assert!(
                self.is_escrow_fulfilled(),
                "[Withdraw]: Can not withdraw when one or more of the parties have not yet paid"
            );

            // Validating the obligation proof to ensure that it is a of a valid resource address
            let obligation_badge: ValidatedProof = obligation_badge
                .validate_proof(self.obligation_non_fungible_resource)
                .expect("[Withdraw]: Invalid obligation proof passed to the method");

            // Getting the data inside the obligation NFT since it will be used to determine the
            // amount of tokens that the caller should get back.
            let obligation: EscrowObligation = obligation_badge.non_fungible().data();

            // Getting the vault containing the funds that the caller should get back.
